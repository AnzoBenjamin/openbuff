...
**Lesson:** deleting a gate without wiring the replacement enforcer into every operation is a regression, not a simplification. Ownership must be _checked at the point of action_ (esp. the mutating kill path), with the owner derived from **trusted run/session state — never from model/tool input**.
...
Parallel editor (impl) + test-writer (tests) against a _sketched_ contract causes drift (job.id vs jobId, chunk vs chunkType/data, matched as boolean vs the matched event, snapshot/wait returning undefined). Write tests against the REAL module, or author impl+tests together.
