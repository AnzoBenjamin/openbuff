...
- Weakening the 7-point `confirmedAnchor` evidence check for _confirming an apply happened_.
...
- **Security note.** The minted token must be scope-bound to `{projectId, path, runId}` exactly like read-minted tokens, and must only be minted when the apply is positively confirmed. Server-side minting from known content is _stronger_ than trusting a client anchor, so this does not lower the security bar.
...
#### #7 — Narrow the whole-file-anchor requirement for the _grant_ (not the _edit_)

- **What.** The 7-point check currently does two jobs: (a) prove the apply succeeded, (b) prove the anchor is whole-file. For granting _sticky read authorization_, arguably only (a) plus a trusted content hash is needed; a confirmed-but-scoped anchor could still authorize reads of that region.
...
- **Security.** Server-side minting must reuse the existing `cap.v3` issuer and scope binding. No new trust in client-supplied anchors is introduced; #1 actually _reduces_ reliance on client anchors.
