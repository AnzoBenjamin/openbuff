...
- _Requirements met:_ R3 (per-line `output` events) and R4 (draining no longer
  depends on `check_job`; an unpolled job still accrues events and settles).
- _Requirement partially met:_ R3's "write-time timestamps". Timestamps are
...
- _Why accepted:_ the piped-stdio edit is the single most safety-critical change
...
- _Requirement met in substance:_ R5's real goal — no 200 ms quantization on a
  match, no dead `wait()` on the shell path — is achieved. `wait()` is now the
  wake mechanism.
- _Residual:_ the periodic re-entry still exists so the loop can re-drain a
...
- _Better than spec:_ one source of truth for the digest shape (the tool's own
  schema/tests), and the gate applies to model-initiated `list_jobs` calls too.
- _Divergent details:_ `truncatedCount` replaces `+N more`; no elapsed bucket;
...
_Residual risk:_ if a job settles and the digest entry carrying its terminal
...
_To implement later:_ persist an unacknowledged-settlement flag per job in the
