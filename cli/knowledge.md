...
- _Knowledge refresh 2026-09-19 (P5 usefulness scorer + tiered retrieval): new pure module `cli/src/services/memory-v2/usefulness-scorer.ts` — `USEFULNESS_SCORE_MODEL_VERSION='usefulness-v1'`, `deriveUsefulnessTier` ('explicit' iff provenance.toolName==='record_decision' AND kind ∈ {decision,constraint}; 'outcome' iff kind==='outcome'; else 'derived'), `ageBucketDays` (whole-day buckets from two recorded wall clocks, cap 90, unparseable/inverted ⇒ 0, never Date.now), `scoreUsefulness` (integer weights w_used=4/w_reinf=6/w_pin=8/w_ignore=3/w_stale=5/w_age=2, counters clamped ≤1e6, final clamp [0,1000], explicit-tier age floor 12 post-clamp; staled/reinforced reserved with no P5 producers). `bun-sqlite-memory-repository.ts` `buildLexicalResult` now folds `observation.reused` events into per-observation used/ignored counters (never reads the memory_usage table — fold events, not tables), derives `asOfTurnWall` as the code-point max occurredAt over the admitted event page (recorded, never ambient), computes tier+integer usefulness per observation candidate, adds a display-only ≤0.05 'reusability' rank contribution (bounded, score still capped at 1.0), and replaces the observation sort with `compareObservations`: retraction → freshnessClass (3 verified+digestFresh, 2 verified, 1 invalid, 0 orphan) → tier (explicit 2 > outcome 1 > derived 0) → integer usefulness → lexical (exact → token → pinned → sourceSequence) → concept slot (constant 0, reserved for P8) → observationId code-point. The old verified sort key dropped as redundant with freshnessClass; matchedTasks keep the original compare; the candidate filter (tokenMatches>0 || exact>0 || pinned) is unchanged — score reorders, never recruits. Ordering is byte-neutral only when tier/freshness/score/retraction all tie; mixed-kind or age-differentiated stores reorder by intended S3 design. Tests: scorer unit suite (10) + retrieval goldens (explicit>discovery at equal freshness, age-floor, neutrality, usage-fold parity) in `bun-sqlite-memory-repository.test.ts`._
