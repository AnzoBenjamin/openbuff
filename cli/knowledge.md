...
- _Knowledge refresh 2026-09-12 (committed-surface hardening): `update_plan_status`'s top-level refine now accepts `requestCommittedSurfaceReview: true` alone (the flag's documented standalone opt-in; the refusal message names the flag). The refinement is check-time only — it does not change the JSON-schema projection or the tool description — so no regenerated tool-definition source is required; schema coverage in `common/src/tools/params/tool/update-plan-status.test.ts`._
