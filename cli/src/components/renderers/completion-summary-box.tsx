...
const deriveTone = (summary: CompletionSummary): Tone => {
  const isBlockingVerdict =
    summary.reviewVerdict === 'BLOCKING' || summary.reviewVerdict === 'NEEDS_WORK'
...
export const CompletionSummaryBox = memo(({ block }: CompletionSummaryBoxProps) => {
  ...
  const hasHooks = summary.hooksPassed > 0 || summary.hooksFailed > 0 || summary.hooksSkipped > 0
  ...
  const filesTone: Tone =
    summary.filesFailed > 0 || summary.rollbackIncomplete > 0 || summary.filesUnconfirmed > 0
      ? 'error'
      : summary.filesRolledBack > 0
        ? 'warning'
        : 'success'
  ...
  const filesText = (() => {
    const parts: string[] = []
    if (summary.filesEdited > 0) parts.push(`${summary.filesEdited} file${summary.filesEdited !== 1 ? 's' : ''} edited`)
    if (summary.filesFailed > 0) parts.push(`${summary.filesFailed} failed`)
    if (summary.filesUnconfirmed > 0) parts.push(`${summary.filesUnconfirmed} unconfirmed`)
    if (summary.filesRolledBack > 0) parts.push(`${summary.filesRolledBack} rolled back`)
    if (summary.rollbackIncomplete > 0) parts.push(`${summary.rollbackIncomplete} rollback incomplete`)
    return parts.join(', ')
  })()
  const hooksText = (() => {
    const parts: string[] = []
    ...
    if (summary.hooksSkipped > 0) parts.push(`${summary.hooksSkipped} skipped`)
    ...
    <HarnessBox tone={tone} gap={1} paddingBottom={1}>
      {hasFiles ? (
        <text style={{ wrapMode: 'word', fg: theme.foreground }}>
          <text style={{ fg: statusColorForTone(filesTone, theme) }}>{filesTone === 'error' ? '❌' : filesTone === 'warning' ? '⚠️' : '✅'}</text>
          <text style={{ fg: theme.foreground }}>{` ${filesText}`}</text>
        </text>
      ) : null}
      {hasHooks ? (
        <text style={{ wrapMode: 'word', fg: theme.foreground }}>
          <text style={{ fg: statusColorForTone(hooksTone, theme) }}>{hooksTone === 'error' ? '❌' : '✅'}</text>
          <text style={{ fg: theme.foreground }}>{` ${hooksText}`}</text>
        </text>
      ) : null}
      {summary.reviewVerdict ? (
        <text style={{ wrapMode: 'word', fg: theme.foreground }}>
          <text style={{ fg: theme.foreground }}>Reviewed:</text>
          <text style={{ fg: statusColorForTone(reviewTone, theme) }}>{` ${reviewIcon(summary.reviewVerdict)}`}</text>
          <text style={{ fg: theme.foreground }}>{` ${summary.reviewVerdict}`}</text>
        </text>
      ) : null}
      {hasTests ? (
        <text style={{ wrapMode: 'word', fg: theme.foreground }}>
          <text style={{ fg: statusColorForTone(testsTone, theme) }}>{testsTone === 'error' ? '❌' : '✅'}</text>
          <text style={{ fg: theme.foreground }}>{` ${testsText}`}</text>
        </text>
      ) : null}
      {hasAux ? (
        <text style={{ wrapMode: 'word', fg: theme.foreground }}>
          <text style={{ fg: statusColorForTone(auxTone, theme) }}>{auxTone === 'error' ? '⚠️' : '✅'}</text>
          <text style={{ fg: theme.foreground }}>{` ${auxText}`}</text>
        </text>
      ) : null}
      {summary.errors > 0 ? (
        <text style={{ wrapMode: 'word', fg: theme.foreground }}>
          <text style={{ fg: theme.error }}>❌</text>
          <text style={{ fg: theme.foreground }}>{` ${errorsText}`}</text>
        </text>
      ) : null}
    </HarnessBox>
  )
