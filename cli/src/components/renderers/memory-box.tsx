...
const statusColorForTone = (tone: 'success' | 'error' | 'warning' | 'secondary', theme: ReturnType<typeof useTheme>) => {
  switch (tone) {
    case 'success':
...
export const MemoryBox = memo(({ block, onInsertCommand = () => {} }: MemoryBoxProps) => {
  ...
  if (block.state === 'status') {
    ...
    const goalText = block.isGoalTruncated && goalExpanded && block.goal ? block.goal : block.goalPreview
    ...
    return (
      <HarnessBox tone={tone} title={header} gap={1} paddingBottom={1}>
        <box style={{ flexDirection: 'column', gap: 0 }}>
          ...
          <text style={{ wrapMode: 'word', fg: theme.foreground }}>{goalText}</text>
          {block.isGoalTruncated ? (
            goalExpanded ? (
              ...
              >
                <text style={{ fg: theme.secondary, wrapMode: 'none' }}>▾ Expand</text>
              </Button>
            )
          ) : null}
        </box>
        <box style={{ flexDirection: 'column', gap: 0 }}>
          <text style={{ wrapMode: 'word', fg: theme.foreground }}>
            <text style={{ fg: theme.foreground }}>{`Decisions: ${block.counts.decisions}`}</text>
            <text style={{ fg: theme.muted }}>{' \u00B7 '}</text>
            <text style={{ fg: theme.foreground }}>{`Requirements: ${block.counts.requirements}`}</text>
            <text style={{ fg: theme.muted }}>{' \u00B7 '}</text>
            <text style={{ fg: theme.foreground }}>{`Edits: ${block.counts.editsMade}`}</text>
          </text>
          <text style={{ wrapMode: 'word', fg: theme.foreground }}>
            <text style={{ fg: theme.foreground }}>{`Validations: ${block.counts.validationResults}`}</text>
            <text style={{ fg: theme.muted }}>{' \u00B7 '}</text>
            <text style={{ fg: theme.foreground }}>{`Blockers: ${block.counts.blockers}`}</text>
            <text style={{ fg: theme.muted }}>{' \u00B7 '}</text>
            <text style={{ fg: theme.foreground }}>{`Next actions: ${block.counts.nextActions}`}</text>
          </text>
        </box>
        <box style={{ flexDirection: 'row', gap: 1 }}>
          <text style={{ wrapMode: 'word', fg: theme.foreground }}>
            <text style={{ fg: theme.secondary }}>Evidence:</text>
            <text style={{ fg: statusColorForTone('success', theme) }}>{` ${block.evidence.fresh} fresh`}</text>
            <text style={{ fg: theme.muted }}>{', '}</text>
            <text style={{ fg: statusColorForTone(staleTone, theme) }}>{`${block.evidence.stale} stale`}</text>
            <text style={{ fg: theme.muted }}>{` (of ${block.evidence.total})`}</text>
          </text>
        </box>
        {block.totalStaleCount > 0 ? (
          staleExpanded ? (
            <box style={{ flexDirection: 'column', gap: 0 }}>
              <text style={{ fg: theme.secondary, wrapMode: 'word' }}>{staleHeader}</text>
              {block.stalePaths.map((p, idx) => (
                <text key={`${p}-${idx}`} style={{ wrapMode: 'word', fg: theme.foreground }}>
                  {p}
                </text>
              ))}
              {block.totalStaleCount > block.stalePaths.length ? (
                <text style={{ fg: theme.muted, wrapMode: 'word' }}>{`+ ${block.totalStaleCount - block.stalePaths.length} more`}</text>
              ) : null}
            ...
            >
              <text style={{ fg: theme.secondary, wrapMode: 'none' }}>{`▾ ${staleHeader}`}</text>
            </Button>
          )
        ) : null}
        {block.evidence.stale > 0 ? (
          ...
          >
            <text style={{ fg: theme.secondary, wrapMode: 'none' }}>Run /memory prune to drop stale evidence entries.</text>
          </Button>
        ) : null}
      </HarnessBox>
    )
  ...
  if (block.state === 'no-record') {
    return (
      <HarnessBox tone="secondary" gap={1} paddingBottom={1}>
        <text style={{ wrapMode: 'word', fg: theme.foreground }}>No persisted task memory to prune for this project.</text>
      </HarnessBox>
    )
  }
  ...
  if (block.state === 'failed') {
    return (
      <HarnessBox tone="error" gap={1} paddingBottom={1}>
        <text style={{ wrapMode: 'word', fg: theme.error }}>{`Memory prune failed: ${block.cause}.`}</text>
        <text style={{ wrapMode: 'word', fg: theme.foreground }}>
  ...
  return (
    <HarnessBox tone="error" gap={1} paddingBottom={1}>
      <text style={{ wrapMode: 'word', fg: theme.error }}>{block.message}</text>
    </HarnessBox>
  )
})
