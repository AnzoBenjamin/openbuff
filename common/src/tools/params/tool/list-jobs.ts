...
Each job includes bucketed pending output relative to the last check_job consumer cursor for process/log output (agent jobs are listed for rediscovery and usually show pending: 'none' for line buckets) and a gap flag when events were truncated from the buffer. Terminal jobs may include a short tail (last ≤10 output lines) and exitCode. The top-level note is declarative (no action required unless you need the output). Use this to rediscover jobIds after context compaction so you can check_job/read_logs/kill_job a shell job or check_background_agent an agent job. This tool takes no agent-supplied input; the owner field is runtime-managed and agents must omit it.
