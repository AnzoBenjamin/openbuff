...
export function createSpecialist(
  ...
  return {
    ...
    spawnerPrompt: config.advisory
      ? config.purpose
      : `${config.purpose} When supplied, params.snapshot_id must be the assigned gate snapshot fingerprint (gate-assigned opaque v3 token) for this spawn; manual spawns omit params.snapshot_id entirely.`,
    inputSchema: {
      prompt: {
        type: 'string',
      ...
      params: {
        ...
        properties: {
          ...
          snapshot_id: {
            type: 'string',
            maxLength: 512,
            // snapshot_id is OPTIONAL: manual spawns omit the key entirely and
            // validate; runtime-owned spawns pass the gate-assigned v3 token.
            // The pattern is enforced only when the key is supplied. Do not
            // apply the pattern when advisory so an empty advisory value is
            // not forced into a hard schema reject.
            ...
            description: config.advisory
              ...
              : 'Optional assigned gate snapshot fingerprint for this spawn — when supplied, the gate-assigned opaque v3:<64-hex> token from the parent gate (params.snapshot_id / specialistCreditFingerprint), not bare hex from get_change_review_bundle.snapshotId. Manual spawns omit this key entirely; runtime-owned spawns echo it exactly as snapshotFingerprint.',
          },
          command: {
            type: 'string',
            maxLength: 4_000,
    ...
    instructionsPrompt: [
      config.advisory
        ...
        : 'When supplied, params.snapshot_id is the authoritative assigned snapshot for this review spawn (opaque v3:… gate-assigned token from the parent gate — not the bare hex snapshotId from get_change_review_bundle). Echo that exact value as snapshotFingerprint. When params.snapshot_id is absent, this is a manual spawn following the omit-for-manual contract (the gate-assigned token exists only for runtime-owned spawns): review the files in params.files and the prompt scope, emit snapshotFingerprint as the empty string, and never invent a v3 token. You may use get_change_review_bundle as read-only evidence (file list/diff/empty-tree check); if a fresh call returns a different bare id, keep reviewing against params.snapshot_id and echo params.snapshot_id — do not emit stale-snapshot solely because the live bundle moved. List the exact normalized project-relative paths you read. Stale-snapshot BLOCKING is only for: inventing a different fingerprint or inability to read the assigned files — not live-bundle drift during review and not an absent snapshot_id on a manual spawn.',
      config.advisory
