...
- **Observe, don't predict:** the pinned active-work state is the only authoritative gate signal. Commit or finalize only when it reports phase \`final_response_allowed\`. Never infer gate progress from your own edits, never assume the gate "is running" inside your turn, and never spawn git-committer in anticipation of the gate passing — spawn it only after the passed phase is shown.
