...
const broadAuditPointer =
  'Broad audit / many-file / coverage-sweep request → read_files `agents/guides/broad-audit.md` before sharding. If that guide is unavailable, still scope first: measure breadth, dispatch one file-picker + one general-agent audit shard per subsystem in bounded waves, and machine-check coverage before synthesizing — never a single codesearch.'
...
export function createBase2(
  ...
  // Cheaper subagents (file-picker) and the orchestrator itself
...
-  **Refactoring Awareness:** Whenever you modify an exported symbol like a function or class or variable, you should find and update all the references to it appropriately using code_search.
...
- **Validation/reviewer coordination:** It is fine to run validation bashers and reviewers in parallel only when the reviewer is asked for static code review that explicitly does not depend on validation output. Always wait for both. Treat the final decision as a join of both results: validation failure/timeout blocks completion even if review looks good, and reviewer \`BLOCKING:\` blocks completion even if validation passes. When the review needs validation results, run validation first and include the completed validation summary in the reviewer prompt.
  ${buildArray(
    ...
    '- Spawn context-gathering agents (file pickers and web/docs researchers) before making edits when the relevant files, APIs, or commands are not already obvious. Use query_index, list_directory, and glob directly for searching and exploring the codebase.',
    isDefault &&
      !planOnly &&
...
[ You spawn another file-picker to find more relevant files, and use code_search and glob tools ]
...
`,
    ...
    handleSteps: function* ({ agentState, prompt, params, config }) {
      ...
      // pairing rationale).
      updateCommittedSurfaceReviewFromMessages(
      ...
      while (true) {
        ...
        if (committedSurfaceRequest?.status === 'pending') {
          ...
          if (!isAttestableSnapshotFingerprint(committedFingerprint)) {
            consumeCommittedSurfaceRequest('rejected', 'non-attestable-fingerprint')
            activeWorkState.latestWorkSummary =
          ...
          const routedCommittedSpecialists = (
            committedOwed.length > 0
              ? Array.from(
                  new Set([...committedBaseRouted, ...committedOwed]),
                )
              : committedBaseRouted
          ).filter(
            (agentType) =>
          ...
          const committedReviewerVerdict = routedCommittedSpecialists
            .map((agentType) => {
              ...
              return committedOutputs[committedOutputs.length - 1]?.verdict ?? ''
            })
            .filter(Boolean)
            .join(', ')
            .slice(0, 400)
...
function deriveCommittedSurfaceFileSet(input: {
    ...
    const cap = typeof input.cap === 'number' && input.cap >= 1
        ? Math.floor(input.cap)
        : 40;
    const runtimeFiles = Array.isArray(input.runtimeFiles) ? input.runtimeFiles : [];
    const seen = new Set<string>();
...
// </gate-helpers-generated>
      ...
      function extractCommittedSurfaceIntent(input: unknown): CommittedSurfaceExtraction {
        const noRequest: CommittedSurfaceExtraction = { claimed: '', requested: false }
        if (!input || typeof input !== 'object' || Array.isArray(input)) {
          return noRequest
        }
        const record = input as Record<string, unknown>
      ...
      function extractCommittedSurfaceReviewRequest(messages: unknown): void {
        ...
        let latest: CommittedSurfaceExtraction = { claimed: '', requested: false }
        for (
      ...
      function updateCommittedSurfaceReviewFromMessages(messages: unknown): void {
