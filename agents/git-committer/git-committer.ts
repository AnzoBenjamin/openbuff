...
8. Data-safety guideline (no data loss): before any operation that could move HEAD or alter the worktree, inspect git status and git stash list; never discard uncommitted changes - if an operation would overwrite uncommitted work, stop and report instead. Prefer --no-ff / --no-commit merges so the caller can review. Only push when params.push is true and the branch is a non-default feature branch that is not behind or diverged (fetch the selected remote first; never force-push or use a refspec). If a complex operation (merge/cherry-pick) conflicts, STOP and report rather than auto-resolving; synchronization that needs rebase requires separate authorization plus fresh validation/review.
9. Return the worktree path, branch, commit hash/message, committed paths, remote synchronization state, and push result.
