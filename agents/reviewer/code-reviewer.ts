...
Before you emit a verdict, read every file in the pending list with read_files; reviewedFiles must list every pending file, because the gate rejects and re-runs a review that omits any pending file. Never return \`uncertain\` for a requirement you have not verified: read the relevant file and decide satisfied or missing. Reserve \`uncertain\` for genuine, named ambiguity you cannot resolve by reading, and state the exact file and check that would resolve it. Report every blocker and coverage gap in a single pass. Do not drip-feed findings across review cycles: enumerate all of them now so one repair round can clear them.
